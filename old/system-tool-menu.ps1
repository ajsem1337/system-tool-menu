...
function PackageManagers {
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            "3" {
                Write-Host "[LOG] Instalowanie Scoop..." -ForegroundColor Green
                $start = Get-Date
                try {
                    Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser -Force
                    Invoke-RestMethod -Uri https://get.scoop.sh | Invoke-Expression
                    Write-Host "Scoop został pomyślnie zainstalowany." -ForegroundColor Green
                } catch {
                    Write-Host "Nie udało się zainstalować Scoopa. Sprawdź połączenie z internetem." -ForegroundColor Red
                }
                $end = Get-Date
                $duration = $end - $start
                Write-Host "[LOG] Instalacja (Scoop) zakończona w $($duration.TotalSeconds) sek."
                Read-Host -Prompt "Naciśnij Enter, aby kontynuować..."
            }
            "4" {
                return
...
function ReinstallMicrosoftStore {
    ...
    try {
        ...
        Write-Host "Błąd! Nie udało się ponownie zarejestrować Sklepu Microsoft." -ForegroundColor Red
        Write-Host "Spróbuj pobrać Microsoft Store z Microsoft Update lub Media Creation Tool." -ForegroundColor Yellow
    }
    $end = Get-Date
    $duration = $end - $start
...
function YesNoAliases {
    Write-Host "[LOG] Ustawianie aliasów dla Tak/Nie..." -ForegroundColor Green
    Set-Alias t yes
    Set-Alias n no
    Write-Host "Aliasy ustawione: 't' dla Tak, 'n' dla Nie." -ForegroundColor Green
}
